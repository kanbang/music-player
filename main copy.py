...
class MainWindow(QWidget, Ui_Form):
    ...
    def show_musics_data(self):
        ...
        self.music_count.setText("<p>歌曲数</p><p style='text-align: right'>%d</p>" % len(self.cur_music_list.musics_table))
    ...
    def set_icon_item(self):
        ...
        if playing_row != -1:
            ...
            if self.stacked_view.currentWidget() == self.music_list_detail:
                self.musics_table.item(playing_row, 0).setText("")
                self.musics_table.setCellWidget(playing_row, 0, icon_label)
    ...
    def on_tb_double_clicked(self, model_index: QModelIndex):
        ...
        self.stop_current()
        self.play()
        self.musics_table.selectRow(index)
    ...
    # 选中歌单列表的音乐, 点击 "播放"
    def on_act_play(self, musics_table: list):
        ...
        self.stop_current()
        if not os.path.exists(self.cur_play_list.get_current_music().path):
            self.next_music()
            return
        self.play()
        self.btn_start.setStyleSheet("QPushButton{border-image:url(./resource/image/暂停.png)}" +
    ...
    # 选中歌单列表的音乐, 点击 "下一首播放"
    def on_act_next_play(self, musics_table: list):
        # 1. 若选中的音乐已在播放列表中, 则移除已存在的, 然后重新加入
        # 2. 选中的音乐依次在current index后加入播放列表
        for music in musics_table:
            self.cur_play_list.remove(music)
    ...
    def on_act_del_from_disk(self, musics_table: list):
        """ 从硬盘上删除本地音乐 """
        # 1.从歌单删除(本地音乐)
        # 2.从播放列表中删除(如果有的话)
        # 3.从硬盘删除
    ...
    def previous_music(self):
        if self.cur_play_list is not None and self.cur_play_list.size() > 0:
            ...
            if not os.path.exists(self.cur_play_list.get_current_music().path):
                Toast.show_(self, "该歌曲已不存在(%s)" % self.cur_play_list.get_current_music().path, False, 3000)
                print(self.cur_play_list.get_current_music().path + "\t不存在")
                self.previous_music()
                return
            self.show_music_info()
            if self.state == self.playing_state:
